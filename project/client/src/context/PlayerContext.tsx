...
// --- Mock Data ---
const mockTracks: UiSong[] = [
  {
    id: "mock-1",
    name: "Night City",
    artist: "Cyberpunk Enthusiast",
    artistId: "artist-1",
...
export const PlayerContextProvider = ({
  ...
  const sendPlayIfNeeded = async () => {
    ...
    if (msPlayed < 5000) return; 
    if (msPlayed > 60 * 60 * 1000) msPlayed = 60 * 60 * 1000; 
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await songApi.getAll();
        const ui = data.map(mapSongToUi);
        // Если API вернуло пустой список, используем моки
        let finalTracks = ui.length > 0 ? ui : mockTracks;
        ...
        console.error("Failed to fetch songs, using mocks:", err);
        setTracks(mockTracks);
      }
    })();
  }, []);
  ...
  const playWithId = async (id: string, customTracks?: UiSong[]) => {
    ...
    if (playStatus) await sendPlayIfNeeded();
  ...
  const changeVolume = (e: any) => {
    const rect = e.currentTarget.getBoundingClientRect();
    const clientX = e.clientX || (e.touches && e.touches[0].clientX);
    const value = Math.min(Math.max((clientX - rect.left) / rect.width, 0), 1);
    setVolume(value);
    localStorage.setItem("player-volume", value.toString());
    setIsMuted(false);
  };
  ...
  useEffect(() => {
    const handler = () => sendPlayIfNeeded();
    window.addEventListener("beforeunload", handler);
    return () => window.removeEventListener("beforeunload", handler);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  return (
    <PlayerContext.Provider
      value={{
        audioRef, seekBg, seekBar, track, tracks, playStatus, time,
        volume, isMuted, isShuffle, repeatMode, setTrack, setTracks,
        play, pause, next, previous, playWithId, seekSong, toggleMute,
        toggleShuffle, toggleRepeat, changeVolume, formatTime, toggleLike,
      }}
    >
      {children}
